...
final class GhosttySurfaceScrollView: NSView {
  ...
  private var scrollbarSyncScheduled = false
  private var coalescedScrollbarUpdateCount = 0
  private var lastScrollbarCoalescingLogTime: CFTimeInterval = 0
  ...
  func updateScrollbar(total: UInt64, offset: UInt64, length: UInt64) {
    ...
    coalescedScrollbarUpdateCount += 1
  ...
  private func scheduleScrollbarSync() {
    guard !scrollbarSyncScheduled else { return }
    scrollbarSyncScheduled = true
    DispatchQueue.main.async { [weak self] in
      MainActor.assumeIsolated {
        self?.flushScrollbarSync()
      }
    }
  }

  private func flushScrollbarSync() {
    scrollbarSyncScheduled = false
    let coalescedCount = coalescedScrollbarUpdateCount
    coalescedScrollbarUpdateCount = 0

    if coalescedCount > 1 {
      let now = CACurrentMediaTime()
      if now - lastScrollbarCoalescingLogTime >= 5 {
        surfaceLogger.debug("Coalesced \(coalescedCount) scrollbar updates for surface \(surfaceView.id)")
        lastScrollbarCoalescingLogTime = now
      }
    }
